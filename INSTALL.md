...
2. **Uygulamayı Başlatın:**
   Proje ana dizininden tek komutla Node 20 LTS kullanılarak başlatılabilir:
   ```bash
   make mobile-run
   ```
   *(Eğer `make mobile-run` yerine manuel başlatmak isterseniz: `cd mobile && PATH="/opt/homebrew/opt/node@20/bin:$PATH" npx expo start --clear`)*
...
## 10. Lint ve Type Checking

```bash
# Tüm dosyaları lint et (uyarılar build'i engellemez)
make lint
...
# Tüm statik analiz (lint + typecheck)
make check
```

Lint kuralları `eslint.config.js` dosyasında tanımlıdır. Kritik hatalar (kullanılmayan ifadeler, duplicate import) **error** seviyesindedir ve build'i engeller. Stilistik ve React kuralları **warn** seviyesindedir — build'i etkilemez, sadece uyarı verir.
